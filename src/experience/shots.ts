 ...
 *
 * LA CHAÎNE : une laque trop proche pour être comprise → des étoiles apparaissent dedans → recul, c'est une voiture →
 * on sort par le reflet et on se retrouve dans le ciel → ce ciel couvre un pays → le pays devient le 06 → une balise →
 * le même véhicule, sale → la lumière le traverse → la laque rend le ciel (la rime) → l'habitacle → le métier →
 * le 06 devient une route → on rattrape et on double un véhicule → il s'arrête → rendez-vous.
 *
...
/** Le plan de la révélation, repris tel quel à la transformation : c'est la rime du récit (promesse, puis preuve). */
const RIME = { position: [9.2, 0.62, 6.8] as Vec3, target: [0.5, 0.92, 0] as Vec3 };
...
export const shots: readonly ShotDefinition[] = [
  {
    ...
    at: 0,
    intent:
      'UNITÉ 1 — Trop près pour comprendre : une surface noire, mouillée, qui occupe tout le cadre. Ce n’est pas une voiture, c’est de la matière. Quelque chose y bouge quand on avance.',
    framing: { position: [3.5, 1.42, 1.5], target: [1.4, 1.0, -0.2], fov: 34, shift: [0.16, 0] },
    ...portrait({ position: [3.4, 1.45, 1.45], target: [1.4, 1.0, -0.2], fov: 40, shift: [0, 0.06] }),
  ...
  },
  {
    ...
    at: 0.55,
    intent:
      'UNITÉ 2 — La caméra glisse le long de la surface : le reflet s’ouvre et des étoiles apparaissent DANS la laque. Aucun texte : le plan se suffit.',
    framing: { position: [2.95, 1.12, 1.15], target: [0.95, 1.06, -0.8], fov: 34, shift: [0.14, 0] },
    ...portrait({ position: [2.9, 1.13, 1.1], target: [0.9, 1.06, -0.8], fov: 40, shift: [0, 0.05] }),
  ...
  },
  {
    ...
    at: 0.5,
    intent:
      'UNITÉ 3 — Le recul, un vrai travelling et non un zoom : c’était une voiture, et elle tient le ciel entier dans sa carrosserie. Sol mouillé au premier plan, falaises et galaxie au fond.',
    framing: { position: [10.4, 0.66, 7.6], target: RIME.target, fov: 38, shift: [0.2, 0] },
    ...portrait({ position: RIME.position, target: RIME.target, fov: 46, shift: [0, 0.14] }),
    via: { desktop: [[4.2, 1.3, 2.8]] as Vec3[], tablet: [[3.2, 1.25, 2.4]] as Vec3[], mobile: [[3.2, 1.25, 2.4]] as Vec3[] },
    pace: { window: [0.02, 0.96], ease: 'inOut' },
    lead: 0.12,
    flight: { fov: 10, roll: 0.03 },
  },
  {
    id: 'ciel',
    chapter: 'ciel',
    at: 0.45,
    intent:
      'UNITÉ 4 — Sortir de la voiture PAR son reflet : la caméra remonte le long de la carrosserie, le regard se lève, le sol s’éloigne. Aucun texte.',
    framing: { position: [4, 260, 240], look: [N, 0.3], fov: 58, shift: [0, 0] },
    ...portrait({ position: [4, 260, 240], look: [N, 0.36], fov: 72 }),
    via: [[8.5, 6.5, 6.4], [5, 62, 84]] as Vec3[],
    pace: { window: [0.02, 0.98], ease: 'inOut' },
    lead: 0.1,
    flight: { fov: 16, shake: 0.6 },
  },
  {
    id: 'univers',
    chapter: 'ciel',
    at: 0.9,
    intent: 'UNITÉ 5 — L’univers : au-dessus d’une mer de nuages, face au cœur de la Voie lactée. Ce ciel-là couvre un pays.',
    framing: { position: [0, 2400, 2800], look: [N, 0.13], fov: 62, shift: [0.06, 0] },
    ...portrait({ position: [0, 2400, 2800], look: [N, 0.2], fov: 78, shift: [0, -0.04] }),
    via: [[3, 900, 1250]] as Vec3[],
    pace: { window: [0.02, 0.98], ease: 'inOut' },
    flight: { fov: 14, shake: 0.8 },
  },
  {
    id: 'france',
    chapter: 'territoire',
    at: 0.4,
    intent:
      'UNITÉ 6 — Le regard bascule vers le sol : la France entière, département par département, posée sur la mer de nuit. Un seul point est allumé, dans les Alpes-Maritimes.',
  ...
  },
  {
    ...
    chapter: 'territoire',
    at: 0.88,
    intent: 'UNITÉ 7 — La descente : les départements voisins s’effacent, le 06 s’allume de la côte aux montagnes, la balise au milieu.',
  ...
  },
  {
    ...
    at: 0.42,
    intent:
      'UNITÉ 8 — Piquer sur la mer, franchir la falaise d’or au ras : au pied de la colonne de lumière, un véhicule attend. C’est le même — mais il ne renvoie plus rien.',
  ...
  },
  {
    ...
    at: 0.88,
    intent:
      'UNITÉ 9 — Au ras du sol, à hauteur de moyeu : la poussière sur l’aile arrière, la roue, et un reflet éteint. La même surface qu’à l’unité 1, mais morte.',
  ...
  },
  {
    ...
    at: 0.18,
    intent: 'UNITÉ 10 — Devant l’optique, à hauteur de pare-chocs : ce que la ligne de lumière va traverser, de l’avant vers l’arrière.',
  ...
  },
  {
    ...
    chapter: 'intervention',
    at: 0.56,
    intent: 'UNITÉ 11 — Le relevé : une ligne d’or traverse la caisse. Derrière elle, la poussière n’est plus là. La caméra recule pour le voir entier.',
  ...
  },
  {
    ...
    chapter: 'intervention',
    at: 0.9,
    intent: 'UNITÉ 12 — Fin du passage, plan rasant le long du flanc : le noir est redevenu profond. Aucun texte.',
  ...
  },
  {
    ...
    at: 0.35,
    intent:
      'UNITÉ 13 — LA RIME : exactement le plan de l’unité 3. La même voiture, le même cadre, le même ciel dans la laque — sauf qu’on sait maintenant ce qu’il a fallu pour l’obtenir.',
    framing: { position: [10.4, 0.66, 7.6], target: RIME.target, fov: 38, shift: [0.2, 0] },
    ...portrait({ position: RIME.position, target: RIME.target, fov: 46, shift: [0, 0.14] }),
  ...
  },
  {
    ...
    chapter: 'transformation',
    at: 0.85,
    intent: 'UNITÉ 14 — Entrer : assis à la place du conducteur, tableau de bord et pare-brise. L’intérieur aussi a été fait.',
  ...
  },
  {
    ...
    chapter: 'prestations',
    at: 0.25,
    intent: 'UNITÉ 15 — Ressortir et prendre un peu de hauteur : le véhicule entier, et la première moitié du métier.',
  ...
  },
  {
    ...
    chapter: 'prestations',
    at: 0.55,
    intent: 'UNITÉ 16 — Tourner autour : trois quarts avant, la carrosserie en fuite, la seconde moitié du métier.',
  ...
  },
  {
    ...
    chapter: 'prestations',
    at: 0.85,
    intent: 'UNITÉ 17 — Reculer dans la nuit : le véhicule propre sous la Voie lactée, la place pour la formule et le déplacement.',
  ...
  },
  {
    ...
    at: 0.9,
    intent:
      'UNITÉ 18 — Le déplacement devient une route : le regard ne quitte pas le cœur de la galaxie, la caméra traverse les nuages, la chaussée se dessine sur le 06 et monte à sa rencontre jusqu’à se poser dans la voie.',
  ...
  },
  {
    ...
    chapter: 'location',
    at: 0.18,
    intent: 'UNITÉ 19 — On roule derrière lui : ses feux allumés, « 1 JOUR » peint sous nos roues, la galaxie au bout de la route.',
  ...
  },
  {
    ...
    chapter: 'location',
    at: 0.44,
    intent: 'UNITÉ 20 — Cent mètres avalés : on se déporte et on le double. Il entre dans le cadre de trois quarts, sa laque prend la galaxie. Aucun texte.',
  ...
  },
  {
    ...
    chapter: 'location',
    at: 0.7,
    intent: 'UNITÉ 21 — Il reprend la tête et s’éloigne : la route converge vers la galaxie, ses feux deviennent deux points rouges.',
  ...
  },
  {
    ...
    chapter: 'location',
    at: 0.93,
    intent: 'UNITÉ 22 — Il s’arrête au bord du 06, nous derrière lui : la falaise d’or et la mer de nuit devant — la place pour les tarifs et les conditions.',
  ...
  },
  {
    ...
    chapter: 'rendezvous',
    at: 0.5,
    intent: 'UNITÉ 23 — L’écran du rendez-vous : la caméra s’immobilise au-dessus du bord du 06 et la nuit s’éteint derrière la carte. Le seul écran sans 3D.',
  ...
  },
  {
    ...
    chapter: 'contact',
    at: 0.6,
    intent: 'UNITÉ 24 — Au bout de la route, au bord du 06 : la mer de nuit et la galaxie devant. La place pour un seul geste.',
